...
more specific extensions (FMA4, XOP, SSE4a). You should use this predef instead
of `HASH_PREDEF_HW_SIMD_X86` to test if those specific extensions have been
detected.
...
//AMD CPUs also use x86 architecture. We first try to detect if any AMD specific
//extension are detected, if yes, then try to detect more recent x86 common
//extensions.

#	undef HASH_PREDEF_HW_SIMD_X86_AMD
#	if !defined(HASH_PREDEF_HW_SIMD_X86_AMD) && defined(__XOP__)
#		define HASH_PREDEF_HW_SIMD_X86_AMD \
...
HASH_PREDEF_DECLARE_TEST(
	HASH_PREDEF_HW_SIMD_X86_AMD, HASH_PREDEF_HW_SIMD_X86_AMD_NAME)
