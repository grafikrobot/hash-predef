...
https://en.wikipedia.org/wiki/Tiny_C_Compiler[Tiny C Compiler] (TinyCC or TCC
for short). Version number available as major, minor, and patch.
...
#	define _HASH_PREDEF_COMP_TCC_HAS_VALUE(VALUE) \
		(_HASH_PREDEF_COMP_TCC_EXPAND(VALUE) != 1)
#	define _HASH_PREDEF_COMP_TCC_EXPAND(VALUE) \
		_HASH_PREDEF_COMP_TCC_CONCAT(VALUE)
...
#		endif
#		define HASH_PREDEF_COMP_TCC_DETECTION \
			HASH_PREDEF_VERSION_NUMBER(0, __TINYC__ / 100, __TINYC__ % 100)
#	else
#		define HASH_PREDEF_COMP_TCC_DETECTION \
			HASH_PREDEF_VERSION_NUMBER(0, 9, 0)
#	endif
