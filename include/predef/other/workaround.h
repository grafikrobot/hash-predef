...
of `HASH_PREDEF_STRICT_CONFIG`. When `HASH_PREDEF_STRICT_CONFIG` is defined this
will expand to a value convertible to `false`. Which has the effect of disabling
all code conditionally guarded by `HASH_PREDEF_WORKAROUND`. When
`HASH_PREDEF_STRICT_CONFIG` is undefine this expand to test the given `symbol`
version value with the `comp` comparison against
...
When `HASH_PREDEF_STRICT_CONFIG` is defined this will expand to a value
convertible to `false`. Which has the effect of disabling all code conditionally
guarded by `HASH_PREDEF_TESTED_AT`. When `HASH_PREDEF_STRICT_CONFIG` is
undefined this expand to either:

- A value convertible to `true` when `HASH_PREDEF_DETECT_OUTDATED_WORKAROUNDS`
is not defined.
...
- A compile error when the expansion of `HASH_PREDEF_WORKAROUND(symbol, >,
major, minor, patch)` is true and `HASH_PREDEF_DETECT_OUTDATED_WORKAROUNDS` is
defined.
...
#		define HASH_PREDEF_TESTED_AT(symbol, major, minor, patch) \
			(HASH_PREDEF_WORKAROUND(symbol, <=, major, minor, patch) ? 1 : (1 % 0))
#	else
#		define HASH_PREDEF_TESTED_AT(symbol, major, minor, patch) \
