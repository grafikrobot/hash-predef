...
The detection is conservative in that it only identifies endianness that it
knows for certain. In particular bi-endianness is not indicated as is it not
practically possible to determine the endianness from anything but an operating
system provided header. And the currently known headers do not define that
...
#	define HASH_PREDEF_ENDIAN_LITTLE_BYTE \
		HASH_PREDEF_VERSION_NUMBER_NOT_AVAILABLE
#	define HASH_PREDEF_ENDIAN_LITTLE_WORD \
		HASH_PREDEF_VERSION_NUMBER_NOT_AVAILABLE
...
#				define HASH_PREDEF_ENDIAN_BIG_BYTE \
...
#		if (defined(__LITTLE_ENDIAN__) && !defined(__BIG_ENDIAN__)) \
			...
			|| defined(__AARCH64EL__) || defined(__loongarch__) \
...
#		if HASH_PREDEF_ARCH_M68K || HASH_PREDEF_ARCH_PARISC \
			|| HASH_PREDEF_ARCH_SPARC || HASH_PREDEF_ARCH_SYS370 \
			|| HASH_PREDEF_ARCH_SYS390 || HASH_PREDEF_ARCH_Z
#			undef HASH_PREDEF_ENDIAN_BIG_BYTE
#			define HASH_PREDEF_ENDIAN_BIG_BYTE \
				HASH_PREDEF_VERSION_NUMBER_AVAILABLE
#		endif
#		if HASH_PREDEF_ARCH_IA64 || HASH_PREDEF_ARCH_X86 \
...
HASH_PREDEF_DECLARE_TEST(
	HASH_PREDEF_ENDIAN_BIG_BYTE, HASH_PREDEF_ENDIAN_BIG_BYTE_NAME)

#include <predef/detail/test.h>
HASH_PREDEF_DECLARE_TEST(
	HASH_PREDEF_ENDIAN_BIG_WORD, HASH_PREDEF_ENDIAN_BIG_WORD_NAME)

#include <predef/detail/test.h>
HASH_PREDEF_DECLARE_TEST(
	HASH_PREDEF_ENDIAN_LITTLE_BYTE, HASH_PREDEF_ENDIAN_LITTLE_BYTE_NAME)

#include <predef/detail/test.h>
HASH_PREDEF_DECLARE_TEST(
	HASH_PREDEF_ENDIAN_LITTLE_WORD, HASH_PREDEF_ENDIAN_LITTLE_WORD_NAME)
