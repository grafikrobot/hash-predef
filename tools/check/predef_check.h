...
int main(int argc, const char ** argv)
{
	...
	for (argi = 1; argi < argc; ++argi)
	{
		...
		if (*exp_val != 0)
		{
			...
			exp_version = HASH_PREDEF_VERSION_NUMBER(
				(unsigned int)atoi(exp_val_a), (unsigned int)atoi(exp_val_b),
				(unsigned int)atoi(exp_val_c));
		}
		for (x = 0; x < generated_predef_info_count; ++x)
		{
			if (*exp_op == 0 && generated_predef_info[x].value > 0
			...
			else if (*exp_op == 0 && generated_predef_info[x].value == 0
				&& strcmp(exp_name, generated_predef_info[x].name) == 0)
			{
				...
				&& strcmp(exp_name, generated_predef_info[x].name) == 0)
			{
				...
				result = 0;
				if (0 == strcmp(">", exp_op)
					&& !(generated_predef_info[x].value > exp_version))
					return argi;
				if (0 == strcmp("<", exp_op)
					&& !(generated_predef_info[x].value < exp_version))
					return argi;
